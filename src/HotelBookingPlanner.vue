...
const hasConflict = (booking) => {
  return bookings.value.some(b => {
    ...
    const bookingEnd = booking.startDay + booking.duration - 1;
    const bEnd = b.startDay + b.duration - 1;
    
    return !(bookingEnd < b.startDay || booking.startDay > bEnd);
  });
};

const wouldConflict = (bookingId, roomId, startDay, duration) => {
  ...
  return bookings.value.some(b => {
    ...
    return !(endDay < b.startDay || startDay > bEnd);
  });
};

const getMaxStartDay = (bookingId, roomId, currentEndDay) => {
  let maxStart = 1;
  ...
  bookings.value.forEach(b => {
    ...
    if (bEnd < currentEndDay) {
      maxStart = Math.max(maxStart, bEnd + 1);
    }
  });
...
const getMaxDuration = (bookingId, roomId, startDay) => {
  let maxDuration = days.value - startDay + 1;
  
  bookings.value.forEach(b => {
    if (b.id === bookingId || b.roomId !== roomId) return;
    
    if (b.startDay > startDay) {
      const possibleDuration = b.startDay - startDay;
      maxDuration = Math.min(maxDuration, possibleDuration);
    }
  });
...
const getBookingStyle = (booking) => {
  ...
  const left = (booking.startDay - 1) * cellWidth + 4;
  const width = booking.duration * cellWidth - 8;
  const opacity = isDragging || isResizing ? 0.7 : 1;
  return `left:${left}px;width:${width}px;top:4px;bottom:4px;background-color:${booking.color};border-color:${booking.color};opacity:${opacity}`;
};
...
const handleMouseMove = (e) => {
  if (dragging.value) {
    ...
    if (booking) {
      let newStartDay = Math.max(1, Math.min(days.value - booking.duration + 1, dragging.value.originalStartDay + daysDelta));
    ...
    if (booking) {
      if (resizing.value.type === 'resize-right') {
        const maxDur = getMaxDuration(booking.id, booking.roomId, booking.startDay);
        const newDuration = Math.max(1, resizing.value.originalDuration + daysDelta);
        const constrainedDuration = Math.min(newDuration, maxDur, days.value - booking.startDay + 1);
        
        if (!wouldConflict(booking.id, booking.roomId, booking.startDay, constrainedDuration)) {
          booking.duration = constrainedDuration;
        }
      } else {
        const originalEndDay = resizing.value.originalStartDay + resizing.value.originalDuration - 1;
        const newStartDay = Math.max(1, resizing.value.originalStartDay + daysDelta);
        const maxStart = getMaxStartDay(booking.id, booking.roomId, originalEndDay);
        const constrainedStartDay = Math.max(newStartDay, maxStart);
        const newDuration = originalEndDay - constrainedStartDay + 1;
        
        if (newDuration >= 1 && !wouldConflict(booking.id, booking.roomId, constrainedStartDay, newDuration)) {
          booking.startDay = constrainedStartDay;
...
const handleMouseUp = () => {
  if (dragging.value || resizing.value) {
    ...
    if (booking && hasConflict(booking) && tempBooking.value) {
      booking.startDay = tempBooking.value.startDay;
...
const addBooking = () => {
  ...
  while (wouldConflict(-1, roomId, startDay, duration) && startDay < days.value) {
    startDay++;
  }
